...
объясняешь услуги и процессы, консультируешь по API платформы и по личным
данным собеседника.
...
async def build_user_context(user: Optional[dict]) -> str:
    ...
    for oid in user.get("orders_created") or []:
        order = await methods.read_order(oid)
...
async def chat_ask(request: ChatRequest, raw: Request) -> dict:
    ...
    if request.credentials:
        found = await methods.login_account(
